...
parser.add_argument('-m', "--num_molecule", type=int, default=1, help="number of particles (unit: k)")
parser.add_argument('-c', "--num_collision", type=int, default=1, help="number of collision times (unit: M)")
args = parser.parse_args()

N = args.num_molecule * int(1e3)
C = args.num_collision * int(1e6)

print("Number of molecules:", N)
print("Number of collisions:", C)
velo_set = np.vstack((np.linspace(1, N, N) * 0.1,
                      np.random.uniform(0.0, np.pi * 2, size=N)))
velo_set = collide(velo_set, C)
...
sn.distplot(velo_set[0, :], rug=True,
            label=str(C) + " times collision -- " + str(N) + " particles")
